...
class Map:
    ...
    def __init__(self, grid = 12):
        ...
        if grid == 36:
            ...
            pass
        self.g = nx.read_shp('../Data/in15oc03/in101503.shp')
    ...
    def GenMap(self,lonmin = -97, latmin = 40, lonmax = -50, latmax = 54, xorig = -2412000, yorig = -1620000):
        ...
        self.mapgen = Basemap(llcrnrlon=self.lonmin, llcrnrlat=self.latmin, urcrnrlon=self.lonmax,
                              urcrnrlat=self.latmax, projection='lcc', 
                              #rsphere = 6370000, 
                              lat_1 = 33, lat_2 = 45, 
    ...
    def pipe(self):
        m = self.mapgen
        m.readshapefile('../Data/CrudeOil_Pipelines_US_EIA/CrudeOil_Pipelines_US_Nov2014','test')
        x0 = self.x[0,:]
        y0 = self.y[:,0]
        ...
        lines = fiona.open('../Data/NaturalGas_InterIntrastate_Pipelines_US_EIA/NaturalGas_Pipelines_US_2015.shp')
        string = []
        for line in lines:
        ...
        m.readshapefile('../GIS emission/PetroleumProduct_Pipelines_US_EIA/PetroleumProduct_Pipelines_US_Nov2014','diesel')
        pipetest = np.zeros((self.nrows,self.ncols))
